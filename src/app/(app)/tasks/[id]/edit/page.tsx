...
export default function EditTaskPage() {
  ...
  const [equipment, setEquipment] = useState<{id: string; name: string; type?: string; make?: string; registration?: string}[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('equipment').select('id, name, type, make, registration').eq('is_active', true).order('name'),
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
        ...
        <select value={equipmentId} onChange={e => setEquipmentId(e.target.value)} className={inputClass}>
          ...
          {equipment.map(eq => (
            <option key={eq.id} value={eq.id}>
              {eq.name}{eq.type ? ' (' + eq.type + ')' : ''}{eq.make ? ' — ' + eq.make : ''}{eq.registration ? ' [' + eq.registration + ']' : ''}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex-1 py-3 bg-aas-blue text-white rounded-xl text-sm font-semibold hover:bg-aas-blue-dark disabled:opacity-60"
        >
