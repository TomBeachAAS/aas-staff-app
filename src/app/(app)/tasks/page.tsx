...
export default async function TasksPage({
  ...
  const { data: assignedTaskIds } = await supabase
    ...
    .select('task_id')
    .eq('user_id', user.id);
  const myTaskIds = (assignedTaskIds ?? []).map(r => r.task_id);
  ...
  const futureTasks = (tasks ?? []).filter(t => !t.task_date || t.task_date > today);
            ...
            <div className="flex gap-2 mt-0.5">
              ...
              {assignees.length > 0 && (
                <p className="text-xs text-gray-400">
                  {assignees.map(a => a.user?.full_name?.split(' ')[0]).join(', ')}
                </p>
              )}
            </div>
          </div>
