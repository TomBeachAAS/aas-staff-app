...
const [{ data: prof }, { data: job }, { data: c }, { data: l }] = await Promise.all([
supabase.from('profiles').select('role').eq('id', user.id).single(),
supabase.from('job_board').select('*').eq('id', id).single(),
...
const manager = ['administrator', 'manager'].includes(prof?.role ?? '');
setIsManager(manager);

if (!job) { router.push('/jobs'); return; }

if (!manager && !(job.created_by === user.id && job.status === 'pending_approval')) {
router.push('/jobs/' + id);
return;
}
...
// Always include the currently-selected location so editing an existing job
// never hides its location from the dropdown (regardless of customer filter).
const filteredLocations = customerId
? locations.filter(l => l.customer_id === customerId || !l.customer_id || l.id === locationId)
: locations;
...
<h2 className="text-lg font-bold text-gray-800 mb-4">Edit job</h2>
...
<button type="button" onClick={() => { setShowNewCustomer(false); setNewCustomerName(''); }} className="flex-1 py-1.5 border border-gray-200 rounded-lg text-xs text-gray-600">Cancel</button>
<button type="button" onClick={handleAddCustomer} disabled={savingCustomer || !newCustomerName.trim()} className="flex-1 py-1.5 bg-aas-blue text-white rounded-lg text-xs font-medium disabled:opacity-60">
...
<input value={newLocationName} onChange={e => setNewLocationName(e.target.value)} placeholder="Location name *" className={inputClass} />
<input value={newLocationPostcode} onChange={e => setNewLocationPostcode(e.target.value)} placeholder="Postcode (optional)" className={inputClass} />
<div className="flex gap-2">
<button type="button" onClick={() => { setShowNewLocation(false); setNewLocationName(''); setNewLocationPostcode(''); }} className="flex-1 py-1.5 border border-gray-200 rounded-lg text-xs text-gray-600">Cancel</button>
<button type="button" onClick={handleAddLocation} disabled={savingLocation || !newLocationName.trim()} className="flex-1 py-1.5 bg-aas-blue text-white rounded-lg text-xs font-medium disabled:opacity-60">
