...
export default async function ReportsPage() {
  ...
  // Aggregate pending expenses (GBP only for simplicity)
  const pendingExpenseGBP = (expensesPendingRows ?? [])
    .filter((e: any) => !e.currency || e.currency === 'GBP')
    .reduce((sum: number, e: any) => sum + Number(e.amount), 0);

  const pendingExpenseForeign = (expensesPendingRows ?? []).filter((e: any) => e.currency && e.currency !== 'GBP');
      ...
      <Card>
        ...
        <CardContent className="space-y-3 pb-4">
          <Row
            ...
            value={`£${pendingExpenseGBP.toFixed(2)}${pendingExpenseForeign.length ? ` + ${pendingExpenseForeign.length} foreign` : ''}`}
            highlight={!!(pendingHolidays?.length)}
          />
          <Row
