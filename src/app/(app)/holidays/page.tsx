...
export default async function HolidaysPage({
  ...
  // Leave year date bounds (April 1 → March 31)
  const leaveYearStart = `${leaveYear}-04-01`;
  const leaveYearEnd = `${leaveYear + 1}-03-31`;
  ...
  // My holidays — filtered by leave year
  ...
  const { data: myHolidays } = await myQuery;

  // Pending holidays (managers only, no year filter — show all pending)
  ...
  const allHolidays = [...(myHolidays ?? []), ...pendingHolidays];
  const allUserIds = [...new Set([
    ...allHolidays.map((h: any) => h.user_id).filter(Boolean),
    ...allHolidays.map((h: any) => h.decided_by).filter(Boolean),
  ])];
  ...
  // Bank holidays for the leave year
  const { data: bankHolidays } = await supabase
    .from('bank_holidays')
    .select('*')
    .gte('date', leaveYearStart)
    .lte('date', leaveYearEnd)
    .order('date');
  ...
  return (
    <div className="p-4 space-y-5 max-w-3xl mx-auto">
      ...
      <p className="text-xs text-gray-400 text-center">
        Allowance: {balance?.allowance ?? 20} days
        {(balance?.adjustments ?? 0) !== 0 && ` (adjusted ${(balance?.adjustments ?? 0) > 0 ? '+' : ''}${balance?.adjustments})`}
      </p>
      ...
      {/* My holiday list — filtered by selected leave year */}
      <Card>
        <CardHeader>
          <CardTitle>My requests — {leaveYearLabel}</CardTitle>
        </CardHeader>
        {(myHolidays ?? []).length === 0 ? (
          <CardContent>
            <p className="text-sm text-gray-400 text-center py-6">No holiday requests for {leaveYearLabel}</p>
          </CardContent>
        ) : (
          <div className="divide-y divide-gray-50">
            {(myHolidays ?? []).map((h: any) => (
      ...
      <Card>
        ...
        <div className="divide-y divide-gray-50">
          {(bankHolidays ?? []).length === 0 ? (
            <CardContent>
              <p className="text-sm text-gray-400 text-center py-4">No bank holidays loaded for this year</p>
            </CardContent>
