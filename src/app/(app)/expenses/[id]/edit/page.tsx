...
if (loading) return <div className="p-4 text-sm text-gray-400">Loading…</div>div>;
  ...
  return (
    ...
    <h2 className="text-lg font-bold text-gray-800">Edit expense</h2>h2>
    </div>div>
      {error && <div className="mb-4 p-3 rounded-lg bg-red-50 text-sm text-red-700">{error}</div>div>}
    ...
    <div>
    <label className="block text-sm font-medium text-gray-700 mb-1">Currency</label>label>
    <select value={form.currency} onChange={e => set('currency', e.target.value)} className={inp}>
      {CURRENCIES.map(c => <option key={c.code} value={c.code}>{c.code}</option>option>)}
    </select>select>
    </div>div>
    <div className="col-span-2">
    <label className="block text-sm font-medium text-gray-700 mb-1">Amount</label>label>
    <div className="relative">
    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">{currencySymbol}</span>span>
    ...
    <div>
    <label className="block text-sm font-medium text-gray-700 mb-2">Receipt photo</label>label>
      {showReceipt ? (
      <div className="relative">
      ...
      <span className="text-xs text-gray-500">Take photo</span>span>
      ...
      <span className="text-xs text-gray-500">Choose file</span>span>
