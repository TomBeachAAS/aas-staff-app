...
const CAN_CONTINUE_OPTIONS = [
  ...
  { value: 'caution', label: '⚠️ With caution' },
...
export default function NewBreakdownPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!equipmentId && !machineName.trim()) { setError('Select a machine or enter its name'); return; }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {equipment.length > 0 && (
            <select
              value={equipmentId}
              onChange={e => { setEquipmentId(e.target.value); if (e.target.value) setMachineName(''); }}
              className={inputClass + ' mb-2'}
            >
              <option value="">— Select from equipment list —</option>
          ...
          {!equipmentId && (
            <input
              ...
              placeholder="Or type machine name if not in list"
              className={inputClass}
            />
          )}
          {equipmentId && (
            <button type="button" onClick={() => setEquipmentId('')} className="text-xs text-aas-blue hover:underline mt-1">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <button
              ...
              className="aspect-square rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center gap-1 text-gray-400 hover:border-aas-blue hover:text-aas-blue transition-colors"
            >
              <Camera size={20} />
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {uploading ? 'Uploading photos…' : saving ? 'Reporting…' : 'Report breakdown'}
          </button>
        </div>
      </form>
    </div>
