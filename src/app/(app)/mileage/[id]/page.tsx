...
export default async function MileageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Rate is stored as a decimal (e.g. 0.45 = £0.45/mile), not pence
  const rateDisplay = `£${Number(claim.rate_per_mile).toFixed(2)} / mile`;
        ...
        <CardContent className="pt-4 pb-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
            ...
            <div>
              ...
              <p className="text-gray-700">{rateDisplay}</p>
            </div>
            {claim.vehicle_reg && (
              <div>
