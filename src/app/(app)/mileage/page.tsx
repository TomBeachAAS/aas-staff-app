...
let query = supabase.from('mileage_claims').select('*').order('claim_date', { ascending: false }).limit(50);
...
<p className="text-sm text-amber-800 font-medium">{submittedCount} mileage claim{submittedCount !== 1 ? 's' : ''} awaiting approval</p>
...
{[{ key: 'mine', label: 'Mine' }, { key: 'submitted', label: 'Submitted' }, { key: 'approved', label: 'Approved' }].map(({ key, label }) => (
<Link key={key} href={'/mileage?filter=' + key} className={cn('flex-1 text-center py-1.5 rounded-md text-xs font-medium transition-colors', filter === key ? 'bg-white text-aas-blue shadow-sm' : 'text-gray-500')}>
{label}
</Link>
))}
</div>
