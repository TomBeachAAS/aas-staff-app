...
export default async function DashboardPage() {
  ...
  // Get my assigned task IDs first
  ...
  // Active tasks I'm assigned to (not completed/cancelled, due today or earlier)
  const { data: myTasks } = myTaskIds.length > 0
    ? await supabase
        .from('tasks')
        .select('*')
        .in('id', myTaskIds)
  ...
  // Completed tasks in last 7 days that I'm assigned to
  const { data: completedTasks } = myTaskIds.length > 0
    ? await supabase
        .from('tasks')
  ...
  return (
    <div className="p-4 space-y-4 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3">
        <a href="https://robotti.agrointelli.com/" target="_blank" rel="noopener noreferrer" className="flex items-center justify-between bg-green-700 hover:bg-green-800 transition-colors rounded-xl px-4 py-3">
          <div>
            ...
            <p className="text-green-200 text-xs">agrointelli.com</p>
          </div>
          <ExternalLink size={16} className="text-green-300 shrink-0" />
        </a>
      ...
      {isManagerOrAdmin && (
        <div className="grid grid-cols-2 gap-3">
          <Link href="/holidays?filter=pending">
            <StatCard label="Pending Approvals" value={pendingApprovals ?? 0} icon={Umbrella} iconColor="text-amber-600" iconBg="bg-amber-50" clickable />
          </Link>
          <Link href="/staff">
            <StatCard label="Staff Off Today" value={staffOffToday ?? 0} icon={CalendarX} iconColor="text-blue-600" iconBg="bg-blue-50" clickable />
          </Link>
          <Link href="/tasks?status=overdue">
            <StatCard label="Overdue Tasks" value={overdueTaskCount ?? 0} icon={AlertCircle} iconColor="text-red-600" iconBg="bg-red-50" clickable />
          </Link>
          <Link href="/expenses">
            <StatCard label="Expenses to Review" value={pendingExpenses ?? 0} icon={TrendingUp} iconColor="text-purple-600" iconBg="bg-purple-50" clickable />
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/jobs?filter=active">
          <StatCard label="Jobs Open" value={openJobsCount ?? 0} icon={Briefcase} iconColor="text-orange-600" iconBg="bg-orange-50" clickable />
        </Link>
        <Link href="/jobs?filter=completed">
          <StatCard label="Completed This Week" value={completedJobsThisWeek ?? 0} icon={CheckSquare} iconColor="text-green-600" iconBg="bg-green-50" clickable />
        </Link>
      </div>
      ...
      <Card>
        <CardHeader className="flex items-center justify-between">
          ...
          <Link href="/tasks?status=mine" className="text-xs text-aas-blue hover:underline">View all</Link>
        </CardHeader>
