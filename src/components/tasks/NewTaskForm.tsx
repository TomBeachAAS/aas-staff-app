...
interface Props {
  ...
  equipment: { id: string; name: string; registration: string | null }[];
}
...
export function NewTaskForm({ userId, initialDate, staff, customers: initialCustomers, locations: initialLocations, equipment: initialEquipment }: Props) {
  ...
  // Filter locations to those linked to selected customer (or unlinked)
  ...
  function handleCustomerChange(newCustomerId: string) {
    ...
    if (locationId) {
      ...
      if (loc?.customer_id && loc.customer_id !== newCustomerId) {
        setLocationId('');
      }
    }
  }
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-5 bg-white rounded-xl border border-gray-100 p-4">
        ...
        {/* Location — filtered by customer when one is selected */}
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="text-sm font-medium text-gray-700">
              Location
              {customerId && filteredLocations.length < locations.length && (
                <span className="ml-1.5 text-xs font-normal text-gray-400">(filtered by customer)</span>
              )}
            </label>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="text-sm font-medium text-gray-700">Equipment / Machine</label>
          ...
          <select value={equipmentId} onChange={e => setEquipmentId(e.target.value)} className={inputClass}>
            <option value="">— none —</option>
            {equipment.map(eq => <option key={eq.id} value={eq.id}>{eq.name}{eq.registration ? ` (${eq.registration})` : ''}</option>)}
          </select>
        </div>
      ...
      {showNewEquipment && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full sm:max-w-sm bg-white rounded-t-2xl sm:rounded-2xl shadow-xl">
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100">
              <h3 className="text-base font-bold text-gray-800">New equipment</h3>
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                ...
                <input value={newEquipName} onChange={e => setNewEquipName(e.target.value)} placeholder="e.g. Robotti 150F" className={inputClass} autoFocus />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
                <input value={newEquipType} onChange={e => setNewEquipType(e.target.value)} placeholder="e.g. Robot, Drone, Tractor" className={inputClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Serial / Registration</label>
                <input value={newEquipReg} onChange={e => setNewEquipReg(e.target.value)} placeholder="Optional" className={inputClass} />
              </div>
            </div>
            <div className="flex gap-3 px-4 pb-4">
