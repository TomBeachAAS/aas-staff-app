...
import { ChevronLeft, ChevronRight, Lock, Check, Plus, AlertCircle } from 'lucide-react';
...
export function TimesheetWeek({
  ...
  function totalWeekHours(): string {
    ...
    Object.values(localEntries).forEach(({ start_time, end_time }) => {
      const m = calcMins(start_time, end_time);
      if (m > 0) total += m;
    });
  ...
  async function saveEntry(dateStr: string) {
    ...
    if (calcMins(entry.start_time, entry.end_time) <= 0) return;
    ...
    <div className="space-y-4">
      ...
      {isLocked && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5 text-sm text-amber-700">
          <Lock size={14} />
          This timesheet has been auto-submitted. Contact your manager to make changes.
        </div>
      )}

      {!isLocked && canEdit && pendingAutoCount > 1 && (
      ...
      <div className="space-y-2">
        {days.map(dateStr => {
          ...
          const mins = startVal && endVal ? calcMins(startVal, endVal) : 0;
          const timeInvalid = !!(startVal && endVal && mins <= 0);
          const hours = fmtMins(mins);
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {!working && (
                    <span className="text-[10px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded font-medium">extra</span>
                  )}
              ...
              {canEdit && (isDirty || entry?.notes) && (
                <div className="mt-2">
                  <input
                    ...
                    disabled={!canEdit}
                    className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs text-gray-600 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-aas-blue disabled:bg-gray-50"
                  />
                </div>
              )}

              {!working && !entry && !isDirty && (
                <button
